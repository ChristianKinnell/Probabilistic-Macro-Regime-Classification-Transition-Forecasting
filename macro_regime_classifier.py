...
Point-in-time, walk-forward macro regime model. Ten stages, matching the
design spec's own numbering (code comments and tearsheet output reference
these same section/step numbers throughout):

1. REGIME DEFINITION:
   Two axes, fit and confirmed entirely independently so a liquidity/vol
   shock is never forced to coincide with a growth/inflation regime change.

   Economic axis -- Growth x Inflation (4 states): Stable Growth,
   Overheating, Stagflation, Contraction/Disinflation. Canonical prototype
   locations in (growth, inflation) space (QUADRANT_PROTOTYPES) seed GMM
   initialization and relabel fitted clusters (Section 5).

   Financial-stress axis -- Volatility x Liquidity/Credit (3 states): Calm,
   Stress, Crisis, ranked ordinally by mean stress score rather than
   prototype-mapped.

   Recession is deliberately not a 4th economic-axis label it's the
   target of a separate supervised NBER-USREC classifier (Section 9/10),
   since folding it into the low-growth/low-inflation quadrant would
   misclassify a stagflationary downturn as non-recessionary.

2. DATA UNIVERSE:
   Nine transparent economic blocks (Config.indicator_blocks()): growth,
   labour, inflation, housing, credit, liquidity, rates, financial
   conditions, volatility ex: INDPRO/PAYEMS/RRSFS (growth), CPIAUCSL/
   PCEPILFE/T5YIE (inflation), M2SL/TEDRATE/WALCL (liquidity), VIXCLS
   (volatility). Each series carries a leading/coincident/lagging/real-time
   timing tag. FEATURE_ADMISSION_CRITERIA documents the inclusion bar
   (economic relevance, coverage, point-in-time availability, reliability,
   incremental information, low structural fragility); BLOCK_ORIENTATION
   fixes sign convention per block. The 14-ETF asset universe is used only
   downstream (Steps 7, 10), never as a classifier input.

3. POINT-IN-TIME DATA ENGINEERING:
   Every series is tracked by observation date, release date and vintage
   date. With a FRED_API_KEY, fetch_fred_vintage_series() pulls the full
   observation x vintage history and reconstruct_asof_history() does the
   as-of join: for decision date t, only vintages released by t are used.
   Without a key, current-vintage data is shifted by a per-series
   publication-lag heuristic instead -- audit_snapshot() reports which path
   was used for every series. The reconstruction is exercised, not just
   available: YoY features (build_pit_feature_column) rebuild BOTH
   numerator and denominator from the vintage known at each checkpoint, so
   later revisions correctly propagate into subsequent YoY readings.

4. FEATURE ENGINEERING & MACRO FACTORS:
   Raw series get block-appropriate transforms (YoY for price/activity
   indices, levels for rates/spreads/ratios), directionally oriented, then
   normalized via rolling standard or robust (median/MAD) z-scores. Level,
   Direction (rolling slope) and Acceleration (its diff) are tracked
   separately per gauge, so "above trend and accelerating" is
   distinguishable from "above trend but decelerating."

   Correlated series within a block compress into one factor via walk-
   forward PCA (pca_composite_gauge_walkforward), refit on an expanding
   window and applied only out-of-sample, with sign consistency enforced
   across refits and loading stability / explained variance tracked
   through time. Sensitivity to window, method and double-standardization
   choices is checked directly rather than assumed. A separate feature set
   (per-series z-score + 3-month momentum, no PCA compression) feeds only
   the Section 9/10 recession validator.

5. LATENT STATE ESTIMATION:
   fit_regime_axis_walkforward() fits both axes the same way: expanding-
   window GaussianMixture (benchmark, n_init=5 or prototype-seeded) and
   GaussianHMM (primary temporal estimator, multi-restart, best-of-
   restarts kept; non-convergence counted and reported, not hidden) at
   each refit checkpoint.

   State identification uses one-to-one Hungarian assignment
   (assign_states_to_prototypes) between fitted means and the canonical
   prototypes, so components can't collapse or get mapped by sign alone;
   distance-to-prototype is tracked and flagged low-confidence beyond a
   threshold. Temporal estimation is strictly causal
   (_hmm_causal_forward_filter: forward-only log-space filtering, no
   smoothing, posterior carried continuously across refits). Full
   posterior vectors are retained throughout, and state-count choice
   (4 and 3) is tested against BIC/AIC/occupancy at alternative k rather
   than assumed. Model hierarchy matches the spec: GMM is the benchmark,
   HMM the primary estimator; Dynamic-Factor-HMM, Bayesian HMM, Markov-
   switching and change-point detection remain challenger architectures,
   stated explicitly as not yet implemented (LIMITATIONS.md).

6. REGIME CLASSIFICATION & CONFIRMATION:
   confirm_regime_sequence() runs a state machine with axis-specific
   thresholds: a candidate must clear an absolute posterior threshold, a
   hysteresis gap over the incumbent's own posterior, a minimum margin
   over the runner-up, and a transition-plausibility score (candidate
   posterior x the fitted incumbent->candidate transition probability) --
   then persist for n_confirm periods before taking over, unless its
   posterior exceeds an override threshold, in which case one period
   suffices.

   Persistence is asymmetric by axis, per spec: the economic axis requires
   multi-period confirmation throughout; the stress axis allows immediate
   Crisis entry (n_confirm=1) but requires 2 confirming periods to exit
   Crisis. Unconfirmed-but-plausible candidates surface as "Transition
   Watch" while the incumbent regime holds. Output includes confirmed and
   candidate state, posterior, margin, persistence count, model agreement,
   and a HIGH/MODERATE/LOW confidence label. The old majority-vote
   smoother is kept only as a diagnostic column, superseded by this
   probability-based logic as the spec calls for.

7. TRANSITION & FORECASTING ENGINE:
   Two independent, cross-checked forecast methods: a structural forecast
   (p_t @ A^h, propagating the current posterior through the fitted
   transition matrix at 1/3/6-month and 1/3-month horizons for the two
   axes) and a feature-conditioned forecast (purged walk-forward
   multinomial logistic regression on gauge Level/Direction/Acceleration,
   posterior, and state duration -- training explicitly excludes any
   example whose label depends on data inside the forecast horizon).
   forecast_agreement() reports disagreement between the two as a
   cross-check.

   End-horizon change risk/destination and first-passage probabilities
   (Monte Carlo path simulation, Config.forecast_mc_paths draws) are both
   computed and reported separately, since "changed by the end of h" and
   "transitioned at some point within h" are different questions. Two
   duration estimates are likewise both reported analytic (1/(1-p_ii)
   from the raw transition matrix) and empirical (survival/hazard curve
   from actual episode durations) -- with the tearsheet flagging when they
   diverge. Recession forecasting reuses the same purged walk-forward
   discipline against NBER USREC directly, with calibration (Brier score,
   log loss, reliability table) tested out-of-sample rather than assumed.

8. CONFIDENCE, UNCERTAINTY & REGIME STABILITY:
   State entropy (current-regime ambiguity) and transition entropy
   (future-path ambiguity) are computed and reported separately, per the
   spec's requirement that confidence-now and confidence-in-the-future are
   different things. Block-bootstrap refits (gmm_membership_bootstrap_ci,
   transition_matrix_bootstrap_ci) give 95% CIs on current state
   membership and every transition-matrix entry, measuring stability of
   today's read under resampling. Factor uncertainty compares current PCA
   explained variance against its historical norm; out-of-distribution
   detection percentile-ranks the current Mahalanobis distance against the
   assigned state's own historical distribution rather than forcing a
   confident label on a novel observation.

   Two fully transparent composite diagnostics -- regime stability (6
   checks: state age, posterior volatility, entropy, margin, agreement,
   recent switches) and state confidence (6-7 checks including cluster fit
   and OOD status) -- report every individual check by name rather than
   collapsing into one opaque score. Current driving z-scores vs. broader
   macro context are shown separately for interpretability, explicitly not
   a causal attribution score.

9. VALIDATION & ECONOMIC REALITY CHECK:
   Four dashboards, matching the spec's four categories:

   A. Classification quality occupancy/duration/episode stats,
   empirical vs. fitted persistence, short-episode rate and the stricter
   genuine-reversion rate (A->B->A within a window), model agreement by
   status, bootstrap state-assignment stability at illustrative dates, and
   NBER recession detection lag.

   B. Forecast quality Brier/log-loss per horizon (both forecast
   methods), transition-risk AUC, destination accuracy, calibration
   (reliability table + logistic slope/intercept), and full recession
   classification metrics. Confusion-matrix-style metrics are used only
   against NBER USREC, per spec -- the four-state taxonomy has no
   observable ground truth to score against directly.

   C. Robustness parameter drift across refits, state-count
   sensitivity, structural-break-era breakdown (named subperiods, not a
   fitted break test), leave-one-gauge-out (stress axis only), rolling-
   vs-expanding comparison, and a standalone naive-vs-point-in-time
   vintage-sensitivity check.

   D. Economic reality held-out variables (labour/housing/rates,
   excluded from clustering inputs) tested via Newey-West HAC for
   consistency with each regime's expected behaviour; four named
   historical event studies reconstructed point-in-time. Cross-asset
   behaviour (Step 10) is reused only as secondary corroboration -- per
   spec, investment performance is never primary evidence the classifier
   is correct.

10. CROSS-ASSET REGIME MAPPING:
   Treated throughout as an APPLICATION of the classifier, not part of
   regime identification asset returns are never a clustering input.

   Regime-conditioned asset profiles (return, vol, Sharpe/Sortino, VaR/
   CVaR, per-episode max drawdown), correlation structure with a Jennrich
   test for statistical distinguishability across regimes, Newey-West HAC
   conditional betas, and a joint Growth x Inflation x Volatility x
   Liquidity matrix are all estimated within-regime over the full sample
   (descriptive, not forecast inputs, so look-ahead isn't a concern here
   the way it is upstream). Forward conditional returns are re-weighted by
   both the current posterior and the forecast posterior, per the spec's
   E[R_i] = sum_k P(S=k)E[R_i|S=k] formulation. Regime information content
   tests, per asset, whether any regime pair's mean-return gap survives
   Holm-Bonferroni correction across the full multiple-testing family --
   economic priors (equities in Stable Growth, duration in Contraction)
   are tested as hypotheses, never hard-coded.

   A regime-conditioned risk model produces a genuinely point-in-time
   VaR/CVaR series where both the lookback window and a leverage cap
   tighten automatically as the confirmed stress state moves Calm ->
   Stress -> Crisis. The walk-forward tactical allocation remains, exactly
   as the spec frames it, a DOWNSTREAM DEMONSTRATION PORTFOLIO -- an
   applied answer to whether regime information has economically
   meaningful portfolio consequences, not the model's claimed edge.
--------------------------------------------------------------------------------
"""
